<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($by == "name") {
            $sql0 = "SELECT client_id, client_name, client_surname, client_account_no FROM iiitv_bank_client
            WHERE client_name LIKE '%".$search."%' OR client_surname LIKE '%".$search."%'
            OR CONCAT(client_name, ' ', client_surname) LIKE '%".$search."%'";
        }
        else {
            $sql0 = "SELECT client_id, client_name, last_name, client_account_no FROM iiitv_bank_client
            WHERE client_account_no LIKE '$search'";
        }
    }
    else {
        $back_button = FALSE;
            ...
            while($row = $result->fetch_assoc()) {
                ...
                <div class="flex-item">
                    ...
                    <div class="flex-item-2">
                        <p id="name"><?php echo $row["client_name"] . " " . $row["client_surname"]; ?></p>
                        <p id="acno"><?php echo "Ac/No : " . $row["client_account_no"]; ?></p>
                    </div>
                    <div class="flex-item-1">
                        <div class="dropdown">
                          ...
                          <div id="dropdown<?php echo $i ?>" class="dropdown-content">
                            ...
                            <a href="./edit_customer.php?client_id=<?php echo $row["client_id"] ?>">View / Edit</a>
                            <a href="./transactions.php?client_id=<?php echo $row["client_id"] ?>">Transactions</a>
                            <a href="./delete_customer.php?client_id=<?php echo $row["client_id"] ?>"
