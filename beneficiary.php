        ...
        <?php
            ...
            while($row = $result->fetch_assoc()) {
                ...
                if (isset($_POST['submit'])) {
                    ...
                    if ($by == "name") {
                        $sql1 = "SELECT client_id, client_name, client_surname, client_account_no FROM iiitv_bank_client
                        WHERE client_id=".$row["recipient_client_id"]." AND (client_name LIKE '%$search%'
                        OR client_surname LIKE '%$search%' OR CONCAT(client_name, ' ', client_surname) LIKE '%$search%')";
                    }
                    else {
                        $sql1 = "SELECT client_id, client_name, client_surname, client_account_no FROM iiitv_bank_client
                        WHERE client_id=".$row["recipient_client_id"]." AND client_account_no LIKE '$search'";
                    }
                }
                else {
                    $sql1 = "SELECT client_id, client_name, client_surname, client_account_no
                             FROM iiitv_bank_client WHERE client_id=".$row["recipient_client_id"];
                }
                ...
                ?>
                    ...
                    <div class="flex-item">
                        ...
                        <div class="flex-item-2">
                            <p id="name"><?php echo $row1["client_name"] . " " . $row1["client_surname"]; ?></p>
                            <p id="acno"><?php echo "Ac/No : " . $row1["client_account_no"]; ?></p>
                        </div>
                        <div class="flex-item-1">
                            <div class="dropdown">
                              ...
                              <div id="dropdown<?php echo $i ?>" class="dropdown-content">
                                ...
                                <a href="./send_funds.php?cust_id=<?php echo $row1["client_id"] ?>">Send</a>
                                <a href="./delete_beneficiary.php?cust_id=<?php echo $row1["client_id"] ?>"
